...
let bestHeroQuestion = function() {
  while(chances > 0) {
    let correctAnswer
    let bestHeros = prompt('Who is one of the best Hero\'s?')
    for(let i = 0; i < heroArray.length; i++) {
      if(bestHeros.toLowerCase() === heroArray[i].toLowerCase()) {
        alert('Yes ' + heroArray[i] + ' is one of the best. ' + heroArray + ' are all valid options.')
        correctAnswer = true
        counter++
        chances = 0
        break
    ...
    if(!correctAnswer) {
      chances--
      alert('Sorry ' + bestHeros + ' is not one of the best, you have ' + chances + ' more chances.')
    }
  }
}
// if(confirm) === array, string, integer, or object, evaluates to true, if (!confirm) === undefined, null, false, or empty string, evaluates to true
let ageQuestion = function() {
  let age = confirm('am I 33?')
  if(age) {
    counter++
  }
}
